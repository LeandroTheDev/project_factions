...
ServerTradeUI.DrawType = {}
ServerTradeUI.LoadType = {}
...
local function OnServerCommand(module, command, arguments)
    if module == "ServerPoints" and command == "getPoints" then
        ServerTradeUI.instance.points = arguments[1]
        Events.OnServerCommand.Remove(OnServerCommand)
    end

    if module == "ServerPoints" and command == "receiveTradeItem" then
        Events.OnServerCommand.Remove(OnServerCommand)
        --Add Items in Inventory
        for i = 0, tonumber(arguments.quantity) - 1 do
            local item = getPlayer():getInventory():AddItem(arguments.target)
            item:setCondition(arguments.condition)
            item:setConditionMax(arguments.maxcondition)
            item:setBroken(arguments.isbroken)
            item:setHaveBeenRepaired(arguments.repaired)
        end
    end
end

function ServerTradeUI:setVisible(visible)
...
function ServerTradeUI.LoadType.ITEM(row, entry)
...
function ServerTradeUI.LoadListings(module, command, arguments)
    if module == "ServerPoints" and command == "loadTrade" then
        ...
        for k, v in pairs(arguments) do
            ...
            scrollingList.doDrawItem = ServerTradeUI.doDrawItem
            ServerTradeUI.instance.tabPanel:addView(k, scrollingList)
            for _, entry in ipairs(v) do
                local row = scrollingList:addItem(entry.type, nil)
                row.type = entry.type
                row.target = entry.target
                row.price = entry.price or 0
                if ServerTradeUI.LoadType[entry.type] then
                    ServerTradeUI.LoadType[entry.type](row, entry)
                else
...
local function OnTick()
    ...
    Events.OnServerCommand.Add(ServerTradeUI.LoadListings)
    sendClientCommand("ServerPoints", "loadTrade", nil)
end

function ServerTradeUI:createChildren()
...
function ServerTradeUI:close()
    self:setVisible(false)
end

function ServerTradeUI:onReload()
...
function ServerTradeUI.BuyType.ITEM(row)
    row.category = selectedCategory
    sendClientCommand("ServerPoints", "buyTrade", { row, nil })
end

function ServerTradeUI.BuyType.VEHICLE(row)
    sendClientCommand("ServerPoints", "buyTrade", { row.price, row.target })
    sendClientCommand("ServerPoints", "vehicleTrade", { row.target })
end

function ServerTradeUI.BuyType.XP(row)
    sendClientCommand("ServerPoints", "buyTrade", { row.price, row.target })
    getPlayer():getXp():AddXP(Perks[row.target], row.quantity, true, false, false)
end

function ServerTradeUI:onBuy()
    ...
    if ServerTradeUI.BuyType[row.type] then
        ServerTradeUI.BuyType[row.type](row)
    end
    Events.OnServerCommand.Add(OnServerCommand)
    sendClientCommand("ServerPoints", "getPoints", nil)
    --Reload
...
function ServerTradeUI.PreviewType.VEHICLE(self)
...
function ServerTradeUI:onPreview()
    if self.preview then
...
function ServerTradeUI:tabPanelRender()
    ...
    for i, viewObject in ipairs(self.viewList) do
        ...
        if viewObject == self.activeView then
            selectedCategory = viewObject.name
...
function ServerTradeUI:addView(name, view)
    local viewObject = {}
...
function ServerTradeUI:doDrawItem(y, item, alt)
    if ServerTradeUI.DrawType[item.type] then
        ServerTradeUI.DrawType[item.type](self, y, item, alt)
    else
        ServerTradeUI.DrawType.DEFAULT(self, y, item, alt)
    end

    return y + item.height
end

function ServerTradeUI:render()
    ...
    if view.mouseoverselected == -1 then
        ...
        if ServerTradeUI.BuyType[row.type] then
            ...
            if self.points < tonumber(row.price) then
        ...
        if ServerTradeUI.PreviewType[row.type] then
...
function ServerTradeUI:new(x, y, width, height)
    ...
    o.title = getText("IGUI_Trade")
    o.available = getText("IGUI_Shop_Available")
    o.serverMsg = getText("IGUI_Trade_Message")
    o.points = 0
    ServerTradeUI.instance = o
    return o
end

return ServerTradeUI
